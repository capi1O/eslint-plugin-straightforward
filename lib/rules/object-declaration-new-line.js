...
module.exports =
{
  ...
  * @returns {void}
  */
		var checkVariableDeclaration = function checkVariableDeclaration(node)
		{
				...
				if (declaration.init && declaration.init.type === "ObjectExpression")
				{
					...
					if (!allowSingleLine)
					{
						if (declarationNameLineNumber === declarationObjectLineNumber)
						{
							context.report(
							{
								...
								fix: function fix(fixer) {return fixer.insertTextBefore(declarationObject, "\n");} });
					...
					// allowSingleLine: true
					else
						{
							...
							var notSingleLine = declarationObject.properties.some(function (property) {return property.loc.start.line !== declarationNameLineNumber;});
